...
const TaskManager: React.FC<TaskManagerProps> = ({ tasks, user, onAdd, onUpdate, onDelete }) => {
  ...
  const handleGenerateSchedule = async (taskId: string, title: string, dueDateStr: string) => {
    setIsGeneratingSchedule(taskId);
    try {
      const startDate = new Date();
      const dueDate = new Date(dueDateStr);

      const newTasks = await geminiService.generateSchedule(title, startDate, dueDate);
      ...
      <div className="flex items-center gap-3 overflow-x-auto pb-4 no-scrollbar">
        {['All', ...Object.values(Category)].map(cat => (
          <button
            ...
            className={`px-6 py-3 rounded-2xl text-sm font-black whitespace-nowrap transition-all ${filter === cat
        ...
        <AnimatePresence>
          {filteredTasks.map((task) => (
            <motion.div
              ...
              key={task.id}
              className={`bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border-2 transition-all relative group flex flex-col justify-between h-full ${task.priority === 'High' ? 'border-rose-50 dark:border-rose-900/20' :
            ...
            >
              <div>
                ...
                <div className="flex items-center gap-3 mb-6">
                  <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{task.category}</p>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-lg z-[100] flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Due Date</label>
                <input
                  type="date"
                  value={newDueDate}
